...
class HttpLib {
    ...
    public function cURL($url, $r) {
        ...
        curl_setopt($handle, CURLOPT_SSL_VERIFYHOST, 0);//206.3.7 by xmb
        curl_setopt($handle, CURLOPT_SSL_VERIFYPEER, 0);//206.3.7 by xmb
        if ($r['redirection'] > 0)
        ...
        if (!empty($str_response)) {
            ...
            if ($curl_error = curl_error($handle))
                echo $curl_error;
//                upf_error($curl_error); //206.3.7 by xmb
            if (in_array(curl_getinfo($handle, CURLINFO_HTTP_CODE), array(301, 302)))
                echo '过多的重定向。';
//                upf_error(__('过多的重定向。')); //206.3.7 by xmb
    ...
    public function fsockopen($url, $r) {
        ...
        // 连接错误
        if (false === $handle)
            upf_error(sprintf('%s: %s', $errno, $errstr));
        // 连接时间超过超时时间，暂时禁用当前方法
