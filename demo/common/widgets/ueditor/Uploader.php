...
//上传七牛云
    private function upQiNiu(){
      $accessKey = $this->config['accessKey'];
      $secretKey = $this->config['secretKey'];
      $bucket = $this->config['bucket'];
      $domain = $this->config['domain'];
      $style = \Yii::$app->params['style'];
      $up = new Qiniu($accessKey,$secretKey,$domain,$bucket);
      $key = time();
      $up->uploadFile($_FILES['upfile']['tmp_name'],$key);
      $url = $up->getLink($key);
      $this->fullName=$url;
      $this->stateInfo = 'SUCCESS';
      $this->fileName = 'img';
      $this->oriName = 'xxx.img';
      $this->fileType = '.jpg';
      $this->fileSize =9999;
//      echo $url;die;
...
//        if(FILE_UPLOAD_QINIU){
//            $this->upQiNiu();
//        }elseif (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
//
